                ...
                <el-form ref="settingsForm" :model="settingsForm" size="large">
                    <el-form-item label="Vars">
                        <el-select v-model="settingsForm.displayVariableImportance" collapse-tags multiple style="float: right">
                            <el-option v-for="item in uniqueDisplayVariableImportance" :key="item.id" :label="item.original" :value="item.feature_name"></el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label="Analysis">
                        <el-select v-model="settingsForm.selectedPlots" collapse-tags multiple style="float: right">
                            <el-option v-for="(itemData, itemIndex) in plotDescriptions" :key="itemIndex" :label="itemData.title" :value="itemIndex">
                                <el-row>
            ...
            <el-col :span="19" style="padding-left: 15px">
                <el-row v-if="Object.keys(plot_data).length > 0 && settingsForm.selectedPlots.length > 0">
                    <el-tabs v-model="activeModelTab">
                        <el-tab-pane v-for="methodName in responseMethods" :key="methodName" :name="methodName">
                                    ...
                                    <el-tabs tab-position="left" v-model="activeModelTabPlotsFeature[imageType + '_png' + '_' + methodName]">
                                        ...
                                        >
                                            ...
                                            <el-tooltip effect="light" placement="top-end" popper-class="download_tooltip">
                                                ...
                                                <img
                                                    v-if="plot_data[imageType + '_png'][methodName][featureName]"
        ...
        return {
            ...
            plotDescriptions: {
                scatter: {
                    title: "Scatter Plot",
                    description:
                        "Scatter plots visualize the relationship between two variables by displaying them as points distributed across a Cartesian plane. They help illustrate how changes in feature values could affect the model's output.",
                },
                heatmap: {
                    title: "Heatmap",
                    description:
                        "Heatmaps depict the interactions between two features by coloring cells according to the model’s output for combinations of feature values, showing how joint variations influence the prediction.",
                },
                ice: {
                    title: "ICE Plot",
                    description:
                        "ICE plots (Individual Conditional Expectation) visualize the change in the prediction outcome as a feature varies while all other features are held constant, highlighting the marginal effect of each feature.",
                },
                lime: {
                    title: "LIME Plot",
                    description:
                        "LIME plots (Local Interpretable Model-agnostic Explanations) explain individual predictions by showing which features were most influential, helping to demystify the model’s behavior on a case-by-case basis.",
                },
                iml_featureimp: {
                    title: "Feature Importance",
                    description:
                        "This plot illustrates the importance of each feature in the predictive model, showing which features have the most significant impact on the model's predictions.",
                },
                iml_interaction: {
                ...
                iml_featureeffect_ale: {
                    title: "ALE Plot",
                    description:
                        "ALE plots (Accumulated Local Effects) visualize how features impact the prediction on average, focusing on how the model’s output changes when a feature varies over its distribution.",
                },
                iml_featureeffect_pdp_ice: {
                    title: "PDP and ICE Plot",
                    description:
                        "This combined plot uses Partial Dependence Plots (PDP) and Individual Conditional Expectation (ICE) plots to show both average and individual feature effects on the model's prediction.",
                },
            },
