<template>
    <div class="app-container">
        ...
        <el-dialog
            :title="$t('views.apps.supervised_learning.analysis.index.dialog.title')"
        ...
        >
            ...
            <span slot="footer" class="dialog-footer">
                <div v-if="errorDialogType === 'server_error'">
                    <el-button @click="cancelInitFeatures">{{ $t("views.apps.supervised_learning.analysis.index.dialog.buttons.cancel") }}</el-button>
                    <el-button type="primary" @click="reinitializeInitFeatures">{{ $t("views.apps.supervised_learning.analysis.index.dialog.buttons.try_again") }}</el-button>
                </div>
                <div v-else-if="errorDialogType === 'general_error'">
                    <el-button @click="errorDialogVisible = false">{{ $t("views.apps.supervised_learning.analysis.index.dialog.buttons.select_another") }}</el-button>
                </div>
            ...
            this.$store.dispatch("setSimonAvailablePackages", this.selectedFiles).then(
                (response) => {
                    ...
                    if (this.avaliablePackages.length < 1) {
                        ...
                        this.errorDialogMessage = this.$t("views.apps.supervised_learning.analysis.index.dialog.messages.error_server");
                ...
                this.$store.dispatch("setSimonAnalysisFeatures", this.selectedFiles).then(
                    (response) => {
                        ...
                        if (Array.isArray(response)) {
                            ...
                            this.errorDialogVisible = true;
                            this.errorDialogMessage = this.$t("views.apps.supervised_learning.analysis.index.dialog.messages.error_format") + featuresExample;
                            this.errorDialogType = "server_error";
                        }
                    },
                    (error) => {
                        ...
                        this.errorDialogVisible = true;
                        this.errorDialogMessage = this.$t("views.apps.supervised_learning.analysis.index.dialog.messages.error_read");
                        this.errorDialogType = "server_error";
                    }
                );
            }
        ...
        checkIsValidOutcome(selectedOutcomes) {
            ...
            if (selectedOutcomes.length > 0) {
                for (let i in selectedOutcomes) {
                    ...
                    if (unique_count < 2) {
                        this.errorDialogVisible = true;
                        this.errorDialogMessage = this.$t("views.apps.supervised_learning.analysis.index.dialog.messages.error_classification");
