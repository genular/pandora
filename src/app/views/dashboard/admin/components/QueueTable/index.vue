<template>
    <div class="queue-list-container" v-loading="queueListLoading" :element-loading-text="$t('globals.page_loading')">
        <div class="filter-container">
            <el-input @keyup.enter.native="handleFilter" style="width: 300px;" class="filter-item" :placeholder="$t('views.dashboard.admin.components.QueueTable.filters.queue_id.placeholder')" v-model="queueFilterQuery.queueID">
            </el-input>
        ...
        <!-- Queue Table -->
        <el-table :data="queueList" ref="queueTable" :empty-text="$t('views.dashboard.admin.components.QueueTable.table.no_data')" @select-all="selectQueue" @select="selectQueue" row-key="queueID" stripe :border="true" fit class="queue-list-container-table" style="width: 100%">
                ...
                <template slot-scope="scope">
                    <div style="display: flex; justify-content: space-between;">
                        ...
                        <div v-if="typeof scope.row.edit['queueName'] !== 'undefined'" style="display: flex; float: right;">
                            ...
                            <el-button style="float: right;" @click="editTableField(scope.row, 'queueName')" :icon="scope.row.edit['queueName'] ? 'el-icon-check' : 'el-icon-edit'" circle>
                            </el-button>
                        </div>
                    </div>
                </template>
            ...
            <el-table-column class-name="settings" fixed="right" :label="$t('views.dashboard.admin.components.QueueTable.table.header.operations')" width="230">
                <template slot-scope="scope">
                    <el-button size="mini" :title="$t('views.dashboard.admin.components.QueueTable.table.operations.info.title')" type="success" plain round icon="el-icon-info" @click="handleOperations('info', scope.row)">
                    </el-button>
                    <el-button size="mini" :title="$t('views.dashboard.admin.components.QueueTable.table.operations.download.title')" type="success" icon="el-icon-download" @click="handleOperations('download', scope.row)">
                    </el-button>
                    <el-button size="mini" :title="$t('views.dashboard.admin.components.QueueTable.table.operations.delete.title')" type="danger" icon="el-icon-delete" @click="handleOperations('delete', scope.row)">
                    </el-button>
                    <el-button size="mini" :title="$t('views.dashboard.admin.components.QueueTable.table.operations.pause.title')" type="warning" plain round icon="el-icon-refresh" disabled @click="handleOperations('toggle', scope.row)">
                    </el-button>
                </template>
            </el-table-column>
        </el-table>
            ...
            <span slot="title">
                <el-row align="top" style="line-height: 32px;">
                    ...
                    <el-col :span="12" style="text-align: right;" v-if="performaceVariables[selectedResampleIDsHash]">
                        <el-select style="padding-right: 25px;width: 100%;" v-model="selectedPerformace" multiple filterable size="small" :placeholder="$t('globals.performanceVariables.placeholder')">
